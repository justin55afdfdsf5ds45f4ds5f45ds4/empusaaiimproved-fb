...
export async function POST(req: Request) {
  try {
    ...
    console.log(`Reference image provided: ${referenceImageUrl ? "Yes" : "No"}`)
    ...
    if (url) {
      try {
        ...
        if (content.length < 50) {
          console.warn("Warning: Extracted content is very short")
        }
      } catch (error) {
    ...
    try {
      ...
      try {
        ...
        console.log(`Parsed response successfully, found ${parsedResponse.posts?.length || 0} posts`)
      } catch (error) {
        console.error("Error parsing OpenAI response:", error)
        console.error("Raw response:", responseText)
        return NextResponse.json(
          {
            error: "Failed to parse AI response. The AI returned an invalid JSON format.",
          },
          { status: 500 },
        )
      }
      ...
      if (postIdeas.length === 0) {
        console.error("No posts were generated by the AI")
        return NextResponse.json(
          {
            error: "The AI didn't generate any posts. Please try again with a different URL or topic.",
          },
          { status: 500 },
        )
      }
      ...
      return NextResponse.json(
        {
          error: `Failed to generate content with AI: ${error instanceof Error ? error.message : String(error)}`,
        },
        { status: 500 },
      )
    }
