...
export async function uploadImageToCloudinary(imageUrl: string, folder = "pinterest", format?: string) {
  const uploadOptions: any = {
    folder,
    resource_type: "image",
  }
  ...
  const res = await cloudinary.v2.uploader.upload(imageUrl, uploadOptions)
  return {
    url: res.secure_url,
    public_id: res.public_id,
  }
